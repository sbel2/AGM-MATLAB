...
Acost = data.Acost(1, :)';  % Transpose to make it a column vector
Bcost = data.Bcost(1, :)';
Dcost = data.Dcost(1, :)';
Arate = data.Arate(1, :)';
Brate = data.Brate(1, :)';
Drate = data.Drate(1, :)';
decoybin = squeeze(data.decoybin(1,:,:)); 
decoybin = decoybin * 10;

% Get the number of trials for this participant
num_trials = length(Acost);
...
% Normalize X.att1
X.att1 = att1(1:530, :);  % Extract the first 530 rows of att1
X.att1 = (X.att1 - min(X.att1)) ./ (max(X.att1) - min(X.att1));  % Min-Max normalization

% Normalize X.att2
X.att2 = att2(1:530, :);  % Extract the first 530 rows of att2
X.att2 = (X.att2 - min(X.att2)) ./ (max(X.att2) - min(X.att2));  % Min-Max normalization
